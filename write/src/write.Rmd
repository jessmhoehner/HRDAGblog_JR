...
Since we are interested in examining only those cases in which people have been reported as dead, we remove all rows where the person's status is "Not_death". For the first set we'll be removing `r table(death1$status)[["Not_death"]]` people and in the second set we'll be removing `r table(death2$status)[["Not_death"]]` people. 

We are also interested in a couple of dates of interest so we created a variable indicating whether or not a death occured prior to or following that date. 
...
```
This pattern appears to suggest that deaths spiked in the beginning of June and end of August into September. However, since these are only from approximately 100 people over 3 months, it is not clear that these numbers are abnormally high since we don't know what normal looks like for this population.
...
In the first dataset, prior to 21 August 21 2019, there was an average of `r mu1pre` deaths reported per day and after 21 August 2019 there was an average of  `r mu1post` deaths reported per day. The mean number of deaths reported following 21 August 2019 is not significantly greater than the mean number of deaths reported prior to 21 August 2019 at the 95% confidence interval (p = 0.86). 

In the second dataset, prior to 21 August 21 2019, there was an average of `r mu2pre` deaths reported per day and after 21 August 2019 there was an average of  `r mu2post` deaths reported per day. The mean number of deaths reported following 21 August 2019 is not significantly greater than the mean number of deaths reported prior to 21 August 2019 at the 95% confidence interval (p = 0.60). 
...
In the first dataset, prior to 21 August 21 2019, there was an average of `r mu1pre` deaths reported per day and a maximum of `r max1_pre` deaths reported per day. The maximum number of deaths reported prior to 21 August 2019 is not significantly greater than the mean number of deaths reported prior to 21 August 2019 at the 95% confidence interval (p = 0.98).

After 21 August 2019 there was an average of  `r mu1post` deaths reported per day and a maximum of `r max1_post` deaths reported per day. The maximum number of deaths reported following 21 August 2019 is not significantly greater than the mean number of deaths reported prior to 21 August 2019 at the 95% confidence interval (p = 0.96). 

In the second dataset, prior to 21 August 21 2019, there was an average of `r mu2pre` deaths reported per day and a maximum of `r max2pre` deaths reported per day.The maximum number of deaths reported prior to 21 August 2019 is not significantly greater than the mean number of deaths reported prior to 21 August 2019 at the 95% confidence interval (p = 1.0).

After 21 August 2019 there was an average of  `r mu2post` deaths reported per day and a maximum of `r max2_post` deaths reported per day. The maximum number of deaths reported following 21 August 2019 is not significantly greater than the mean number of deaths reported prior to 21 August 2019 at the 95% confidence interval (p = 0.96). 


## Acknowledgements {-}

Learning HRDAg's data analysis process has been an incredible learning opportunity for me. Looking at projects from the very macro ("How many tasks do I have?") to the very micro ("Can I pipe this?") levels is a fundamental shift in the way I had been working and I know I will take this into my future projects. 

this project architecture structure discussed in this post, with discrete tasks using Makefiles, was developed by Drs. Scott Weikart and Jeff Klingner and formalized around 2007. I’m particularly grateful to Dr. Patrick Ball, who motivated the post, helped me think through the concepts, and provided valuable feedback.
