...
For this we first need to account for the days on which we do not have any reported deaths by adding these dates back in.

Next, we need to obtain the mean frequency of deaths per day before and after the dates of events of interest. 

Finally, in order to test our hypothesis we use the stat's package's ppois function to test the likliehood that the frequency of deaths we are observing is within the range of expected variation. Since we want to know specifically if the frequency of deaths we are observing is greater than the expected number of deaths we perform a one-tailed test. 
