...
write_delim(death1, files$output1, delim="|", na= "NA")
...
death2 <- readr::read_delim(files$input2, delim="|") %>%
  ...
  mutate(DOD = as.Date(`date`, "%m/%d/%Y")) %>%
  mutate_at(vars(DOD,sex,status), as.factor)
write_delim(death2,files$output2, delim="|", na= "NA")
...
In order to determine if the number of deaths reported is abnormally high, we use an epidemiological (or epi) curve to see if this number of deaths per day appears to spike anywhere. Epi curves are often used in the investigation into whether an outbreak of an outcome has occured and can even suggest a means of propogation; curves for infections transmitted from person to person have different shapes than curves of diseases transmitted from a water source, for example. In our data, if an epidemic of deaths were occuring, we would expect to see a spike in the number of deaths reported per day. The day on which most deaths were reported in the first set was `r getmode(death1$DOD)` and in the second set was `r getmode(death2$DOD)`. It is unclear whether or not these would be high enough to be considered spikes.

It looks like we have some people recorded who are not deaths, since we are only interested in the number of deaths over time, we will remove all rows where the person's status is "Not_death". For the first set we'll be removing `r table(death1$status)[["Not_death"]]` people and in the second set we'll be removing `r table(death2$status)[["Not_death"]]` people.
...
#retain only people reported as dead
death1 <- filter(death1, status == "dead")

#format date of death to a date
death1$DOD<- as.Date(death1$DOD)

# compute daily incidence
date_case1<- incidence(death1$DOD, interval = 1 )
...
#format date of death to a date
death2$DOD<- as.Date(death2$DOD)
stopifnot(is.Date(death2$DOD))

# compute daily incidence
date_case2<- incidence(death2$DOD, interval = 1 )
...
In order to establish whether or not these data suggest spikes in numbers of deaths reported in a given day we construct our hypothesis to ask: given the mean number of deaths reported per day, are the days on which we are seeing more deaths within the expected variation around the observed mean? Specifically, on the days when we are seeing what appear to be a high number of deaths per day, are these numbers statistically significantly greater than we would expect with normal variation? 

For this we first need to account for the days on which we do not have any reported deaths by adding these dates back in with counts of zero.
#working on this

Next, we need to obtain the number of deaths occuring on each day or the frequency of deaths/day.

Fianlly, in order to test our hypothesis we use the stat's package's ppois function to test the likliehood that the frequency of deaths we are observing is within the range of expected variation. Since we want to know specifically if the frequency of deaths we are observing is greater than the expected number of deaths we perform a one-tailed test. 

For example, if the mean number of deaths reported per day is 4 and we're seeing days with 7 deaths reported per day, what is the likelihood of having days with 7 or more deaths reported per day?  

## Hypothesis testing
``` {r hypothesistest, message = FALSE}
#add in counts of zero for days on which no deaths were reported
death1 %>%
...
counts1 <- count(death1, death1$DOD)
mucpd1 <- mean(counts1$n)

#max number of deaths/day
maxdeath1 <-max(counts1$n)

#mean number of deaths /day in set 2 
counts2 <- count(death2, death2$DOD)
